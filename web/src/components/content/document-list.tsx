...
export function DocumentList({
  ...
  if (!documents.length) {
    return (
      <p className="rounded-md border border-dashed border-ink-300 bg-ink-50 px-6 py-12 text-center text-ink-500">
        {tReports("empty")}
      </p>
    );
  }
    ...
    <div>
      ...
      <div className="mb-10 flex flex-wrap items-center gap-3">
        ...
        <select
          ...
          className="h-11 min-w-[180px] rounded-md border border-ink-300 bg-white px-3 text-[15px] text-ink-800 transition-colors hover:border-brand-600 focus:border-brand-600"
        >
                ...
                <div key={y}>
                  ...
                  <ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                    {docs.map((doc) => (
                      <li
                        key={doc.id}
                        className="rounded-md border border-ink-200 bg-white p-4 transition-shadow hover:shadow-md"
                      >
...
export function DocActions({
  ...
  const cls = cn(
    "inline-flex items-center gap-2 rounded-md border border-ink-300 bg-white font-medium text-ink-700",
